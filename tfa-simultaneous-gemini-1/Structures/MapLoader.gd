...
@export var map_image_path: String = "res://Maps/cemetery.png" # need to make configurable in code
...
# player spawns, npcs spawns (dict on world state?) 
# items spawn (i guess same considerations?
	...
	for y in height:
		for x in width:
			...
			if structure_mask[idx] == 1:
				...
				# Near structure edge (t≈0): more of struct_img
				# Far from structure (t≈1): more of clean_img
				# Wait — we want the opposite for the underneath layer:
				# The underneath should match the structure map near edges
				# so there's no seam when structures are present
				blended = clean_color.lerp(struct_color, 1.0 - t)
