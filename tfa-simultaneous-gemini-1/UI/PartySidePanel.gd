...
func _create_character_panel(character, index: int) -> Dictionary:
	...
	hp_bar.max_value = max(1, character.max_hp)
	hp_bar.value = character.current_hp
	hp_bar.show_percentage = false
	hp_bar.mouse_filter = Control.MOUSE_FILTER_IGNORE
	var hp_fill = StyleBoxFlat.new()
	hp_fill.bg_color = HP_COLOR_FULL
	hp_bar.add_theme_stylebox_override("fill", hp_fill)
	var hp_bg = StyleBoxFlat.new()
	hp_bg.bg_color = Color(0.3, 0.1, 0.1, 0.6)
...
func _update_health_bar(data: Dictionary, character) -> void:
	var bar: ProgressBar = data["hp_bar"]
	var fill_style: StyleBoxFlat = data["hp_fill"]
	var max_hp: int = max(1, character.max_hp)
	bar.max_value = max_hp
	bar.value = character.current_hp
	var pct: float = float(character.current_hp) / float(max_hp)
