	...
	match current_state:
		...
		AttackState.CAST_WINDUP:
			...
			if attack_timer >= cast_windup_duration:
				...
				attack_timer = 0.0
				emit_signal("attack_hit_frame") # Signal to spawn the projectile
...
# Note: In your IK system, (0, -Length) is roughly "Straight forward" relative to the shoulder
func _animate_cast_windup():
	var ease_in = _ease_in_quad(attack_progress)
	var mirror = _get_hand_mirror()

	# Pull hand back slightly towards chest (cocking the spell)
	target_arm_offset = _mirror_offset(Vector2(-5, 10)) * ease_in
	target_weapon_rotation = 0.0 # Keep hand relatively steady
	# Rotate body slightly away from casting hand
	target_body_rotation = (-0.1 * ease_in * mirror)

func _animate_cast_release():
	var ease_out = _ease_out_cubic(attack_progress)
	var mirror = _get_hand_mirror()

	# PUSH hand straight out. 
	# Assuming -Y is "forward" along the arm/body axis in your setup:
	# We push it out significantly to fully extend the IK chain.
	var start_pos = _mirror_offset(Vector2(-5, 10))
	# Target: Straight out (0 X) and far forward (-Y)
	var end_pos = _mirror_offset(Vector2(0, -45)) 

	target_arm_offset = _second_order_vec2(start_pos, end_pos, ease_out)
	target_body_rotation = (0.2 * ease_out * mirror) # Twist body into the cast

func _animate_cast_recovery():
	var ease_out = 1.0 - _ease_out_quad(attack_progress)
	var mirror = _get_hand_mirror()

	# Return from extended position
	target_arm_offset = _mirror_offset(Vector2(0, -45)) * ease_out
	target_body_rotation = (0.2 * ease_out * mirror)

	# Helper to start casting (called by Character)
func start_cast():
