...
func get_valid_actions(board_state):
	...
	var queen_dirs = [
		Vector2(1, 0), Vector2(-1, 0), Vector2(0, 1), Vector2(0, -1), # Rook
...
# Death Rattle: Leave battlefield for one turn
func on_capture(game_board):
	last_known_pos = grid_position
	get_parent().remove_child(self)
	game_board.phased_out_pieces[self] = 2
