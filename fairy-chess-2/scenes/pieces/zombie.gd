...
func get_valid_actions(board_state):
	...
	if actions.size() > 0:
		var preferred_action = actions[actions.size()-1]
		print("preferred automatic action for zombie: ", preferred_action)
		game_board.automatic_actions[self] = preferred_action

	else: 
		print("no valid actions for zombie")
	#use piece instead of string and put this whole thing in resolve in gameboard instead of in here.
	 #should declare an automatic action
		
	return {} #only moves automatically
